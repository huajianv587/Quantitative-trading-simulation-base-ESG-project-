...
def main() -> int:
    ...
    stdout_log = temp_dir / f"esg_p2_stack_{args.port}.log"
    stderr_log = temp_dir / f"esg_p2_stack_{args.port}.err.log"
    ...
    try:
        ...
        status_code, status_payload = request_json("GET", f"{base_url}/api/v1/quant/p2/status", timeout=args.request_timeout)
        report_code, report_payload = request_json(
            ...
            body={
                ...
                "benchmark": "SPY",
                "capital_base": 500000,
                "research_question": "Run the P2 graph + strategy selector smoke test.",
            },
        )
        ok = (
            status_code == 200
            ...
            and "selector" in status_payload
            and "strategy_selector" in report_payload
            and "graph_summary" in report_payload
        )
        emit_json(
            {
                "ok": ok,
