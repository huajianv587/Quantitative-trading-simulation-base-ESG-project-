...
const COPY = {
  ...
  zh: {
    title: '市场雷达',
    subtitle: '汇总免费新闻、行情、SEC 与本地 ESG 证据流。',
    scan: '扫描雷达',
    refresh: '刷新证据',
    provider: '来源过滤',
    symbol: '股票过滤',
    ...
    <div class="workbench-page live-page market-radar-page">
      ...
      <section class="grid-2 market-radar-layout workbench-main-grid">
        <article class="card">
          ...
          <div class="card-body" id="market-radar-feed">${emptyState('Loading evidence')}</div>
        </article>
        <article class="card">
          <div class="card-header"><span class="card-title">${c('quality')}</span></div>
          <div class="card-body" id="market-radar-summary">${emptyState('No scan yet')}</div>
        </article>
      </section>
    </div>`;
  renderFieldPreviews();
...
async function scanRadar() {
  ...
  setLoading(feed, 'Scanning free-tier providers...');
  try {
    ...
    _lastLineage = payload.lineage || ['Free-tier scan', 'Evidence normalization', 'As-of guard', 'Decision-ready feed'];
...
async function refreshEvidence() {
  const feed = _container.querySelector('#market-radar-feed');
  setLoading(feed, 'Loading evidence lake...');
  try {
  ...
  return `
    ...
    <div class="radar-feed-footer">
      <div class="workbench-mini-grid radar-feed-mini-grid">
        ${miniMetric('Page', `${_view.page}/${pageCount}`)}
        ${miniMetric('Visible', `${pageItems.length}`)}
        ${miniMetric('Top q', num(topQuality))}
        ${miniMetric('Lead', esc(leadSymbol))}
      </div>
      <div class="workbench-kv-list compact-kv-list radar-feed-footer__list">
        <div class="workbench-kv-row"><span>Page providers</span><strong>${esc(pageProviders.join(', ') || '-')}</strong></div>
        <div class="workbench-kv-row"><span>Decision lane</span><strong>${pageItems.some((item) => String(item.leakage_guard || '').includes('safe')) ? 'as-of safe' : 'review'}</strong></div>
      </div>
      <section class="radar-selected-evidence">
        <div class="workbench-section__title">Selected Evidence</div>
        ...
        <div class="workbench-mini-grid radar-feed-mini-grid">
          ${miniMetric('Confidence', num(selected?.confidence || selected?.quality_score || 0))}
          ${miniMetric('Quality', num(selected?.quality_score || selected?.confidence || 0))}
          ${miniMetric('Freshness', num(selected?.confidence || 0))}
          ${miniMetric('Lineage', selected?.provider ? 'linked' : 'pending')}
        </div>
...
function renderItems(payload = {}) {
  ...
  const freshnessValue = filtered.length
    ? Math.max(...filtered.map((item) => Number(item.confidence || item.quality_score || 0)))
    : 0;
  ...
  _container.querySelector('#market-radar-summary').innerHTML = `
    <div class="quality-summary-panel">
      <div class="workbench-metric-grid">
        ${metric('Items', filtered.length, 'positive')}
        ${metric('Providers', providersCount)}
        ${metric('Symbols', symbolsCount)}
        ${metric('Avg quality', num(avgQuality))}
      </div>
      ...
      <section class="workbench-section">
        ...
        <div class="workbench-kv-list compact-kv-list">
          ...
          <div class="workbench-kv-row"><span>${c('provider')}</span><strong>${esc(providers().join(', ') || 'all')}</strong></div>
          <div class="workbench-kv-row"><span>${c('symbol')}</span><strong>${esc(symbolFilter() || 'all')}</strong></div>
      ...
      <section class="workbench-section">
        ...
        <div class="preview-step-grid">
          ${lineage.slice(0, 4).map((step) => `<div class="preview-step"><span>${esc(step)}</span><strong>safe</strong></div>`).join('')}
          <div class="preview-step"><span>Frozen paper inputs</span><strong>untouched</strong></div>
        </div>
      </section>
      <section class="workbench-section">
        <div class="workbench-section__title">Latest item</div>
        <div class="workbench-kv-list compact-kv-list">
          <div class="workbench-kv-row"><span>Symbol</span><strong>${esc(latest?.symbol || '-')}</strong></div>
          <div class="workbench-kv-row"><span>Provider</span><strong>${esc(latest?.provider || '-')}</strong></div>
          <div class="workbench-kv-row"><span>Type</span><strong>${esc(latest?.item_type || '-')}</strong></div>
          <div class="workbench-kv-row"><span>Confidence</span><strong>${num(latest?.confidence || latest?.quality_score || 0)}</strong></div>
        </div>
      </section>
    </div>`;
}
